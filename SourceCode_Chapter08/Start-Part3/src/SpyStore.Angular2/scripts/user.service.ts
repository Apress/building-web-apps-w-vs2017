...
export class UserService {
    ...
    getUsers(): Observable<User[]> {
        return this.http.get(this.config.apiEndpoint + "customer").map(response =>
            (<User[]>response.json()));
    }
    ...
    ensureAuthenticated(): Observable<User> {
        var observable = new Observable((subscriber) => {
            if (this.IsAuthenticated) {
                ...
                return this.http.get(this.config.apiEndpoint + "customer").map(response => <User[]>response.json()).subscribe(users => {
                    this.User = users[0];
                    subscriber.next(this.User);
                });
            }
